...
- **Phase 3.B DONE 2026-05-03 (commit pending) — Audio gating + TTS decision doc**: Gated `MeditationBellPlayer.play()` on `audio != SILENT` in `MeditationEntryPoint` (was firing regardless of mode — real bug; SILENT did nothing). Wrote `.claude/MEDITATION_TTS_DECISION.md` surveying 4 candidate TTS architectures (pre-generated audio bundle / Android system TTS / Sherpa-ONNX local neural / defer voice) with cost/latency/quality/APK trade-offs. Recommends **Option A** (pre-generated audio via ElevenLabs, ~13 MB APK overhead for 6 Latin langs at v1, ~25 MB for all 12). Existing chat TTS (Gemini cloud, `GeminiNativeVoiceSystem`) is wrong-shaped for meditation: 75 utterances/session × cloud round-trip = $$$ + latency that breaks breath sync. VOICE mode currently behaves as CHIMES until you pick the approach.
- **Phase 3.B' BLOCKED on user decision**: TTS implementation awaits choice in `.claude/MEDITATION_TTS_DECISION.md` — voice character / TTS provider / launch lang scope / coach speak policy.
- **Phase 3.C (final)**: Cross-locale screenshot regression (5 phases × 6 Latin) + marketing screenshots for Play Store listing.
