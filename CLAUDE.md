...
- **Phase 3.A DONE 2026-05-03 (commit pending) — A11y batch**: Reduced-motion expect/actual (`core/ui/.../accessibility/ReducedMotion.kt` — Android `Settings.Global.ANIMATOR_DURATION_SCALE`, iOS `UIAccessibilityIsReduceMotionEnabled`, desktop stub) → `BreathingPacer` clamps tweens to 200ms + static ambient layer; keyboard shortcuts (ESC=stop, SPACE=pause) via `focusable + onPreviewKeyEvent` + autofocus; TalkBack — pacer geometry hidden, cue word `liveRegion = Polite`, count hidden (no countdown spam). Build green.
- **Phase 3.B (next)**: TTS voice guidance via Sherpa-ONNX (gated on audio=VOICE) — cue words at segment start + coach lines at sub-phase transitions + volume ducking. Reuses existing chat TTS pipeline.
