...
- **i18n state (aggiornato 2026-04-19, Fase A DONE)**:
  ...
  - **Tier 3.C-E ancora deferred** (sessioni future, refactor profondo): `BlockType` + `BlockResolution` enum in `core/util` con `displayName: String` IT (~13 strings, stesso circular-dep pattern di `Trend`) + `BlockScreen.kt` action items per-block-type (~20 IT advice strings) + `JournalHomeScreen.getDailyPrompt + getMoodFollowUp` (13+ time-of-day prompts in non-Composable) + wellness wizard screens (BrainDump/Gratitude/EnergyCheckIn/Movement/SleepLog/Connection/Inspiration/Reframe ~80 strings) + `buildWeeklyReflection` in PercorsoScreen (paragraph builder, ~12 sentences mood/stress/resilience).
