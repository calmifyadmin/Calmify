...
- **i18n state (aggiornato 2026-04-19, Fase A DONE)**:
  ...
  - **Tier 3 deferred** (architectural refactor, sessione futura): `DateFormatters.kt` (getTimeOfDayGreeting/formatSectionHeader/getWeekLabel/getDayLabel/getFullDayName non-Composable utilities) + `JournalHomeScreen.kt::getDailyPrompt` (13+ time-of-day prompts in non-Composable) + `CalculateStreaksUseCase.dayOfWeekItalian` (cosmetico, MonthlyStats.mostProductiveDay non visibile UI) + wellness wizard screens (BrainDump/Gratitude/EnergyCheckIn/Movement/SleepLog/Connection/Inspiration/Reframe ~80 strings) + BlockScreen action items (~20) + HabitListScreen save labels.
