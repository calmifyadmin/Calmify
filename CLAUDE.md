...
- **i18n state (aggiornato 2026-04-19, Fase A DONE)**:
  ...
  - **Tier 3.D + 3.F STILL deferred** (sessioni future): 3.D wellness wizards (BrainDump/Gratitude/EnergyCheckIn/Movement/SleepLog/Connection/Inspiration/Reframe ~80 strings/8 file) + per-enum displayName fields (ConnectionType/EnergyType/MovementType/SleepDisturbance/Trigger/Sensation/gratitudeCategory/ReframeCategory — same circular-dep pattern, drop displayName + inline when). 3.F Garden activity expanded card (long body + 3 benefits per ognuna delle 19 attività + "Benefici" header + "Inizia Attivita'" CTA) ~78 keys × 6 langs ≈ 468 traduzioni — refactor: `data class GardenActivity` aggiungere `longDescriptionRes` + `benefitsRes: List<StringResource>`.
