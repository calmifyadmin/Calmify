...
class GetAchievementsUseCase(
    ...
    private fun calculateBadgeProgress(
        ...
        val daysUsing = if (oldestEntry != null) {
            val oldest = Instant.fromEpochMilliseconds(oldestEntry.dateMillis)
                .toLocalDateTime(TimeZone.currentSystemDefault())
                .date
            (today.toEpochDays() - oldest.toEpochDays())
        } else 0
    ...
    private fun calculatePositiveStreak(insights: List<DiaryInsight>): Int {
            ...
            .groupBy { insight ->
                try {
                    if (insight.dayKey.isNotBlank()) {
                        ...
                        Instant.fromEpochMilliseconds(insight.generatedAtMillis)
                            .toLocalDateTime(TimeZone.currentSystemDefault())
                            .date
                            .toString()
                    }
        ...
        for ((date, avgSentiment) in dailySentiments) {
            ...
            } else if (date < expectedDate.minus(1, DateTimeUnit.DAY)) {
                break
            }
        }
    ...
    private fun countEntriesByHourRange(
        ...
        return diaries.count { diary ->
            try {
                val hour = Instant.fromEpochMilliseconds(diary.dateMillis)
                    .toLocalDateTime(TimeZone.currentSystemDefault())
                    .hour
