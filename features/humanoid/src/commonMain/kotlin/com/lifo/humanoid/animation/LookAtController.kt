...
class LookAtController {
    ...
    fun update(cameraEye: FloatArray, deltaSeconds: Float): Map<String, Float> {
        ...
        val rawYaw = ((180.0 / PI) * atan2(dx.toDouble(), (-dz).toDouble())).toFloat()

        // Pitch: atan2(dy, horizontalDist)
        // Positive pitch -> camera is ABOVE
        val rawPitch = ((180.0 / PI) * atan2(dy.toDouble(), horizontalDist.toDouble())).toFloat()
        ...
        if (enableSaccades) {
            ...
            if (saccadeTimer > saccadeMinInterval && Random.nextFloat() < saccadeProc) {
                saccadeTargetYaw = (2f * Random.nextFloat() - 1f) * saccadeRadius
                saccadeTargetPitch = (2f * Random.nextFloat() - 1f) * saccadeRadius
                saccadeTimer = 0f
            }
