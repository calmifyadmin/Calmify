...
**Bio-Signal Phase 4 DEPLOYED + DEVICE-VERIFIED 2026-05-12**: Pipeline end-to-end live. Sequence:
1. Merge `design-system-refactor` → `backend-architecture-refactor` fast-forward (10 commits, 76 file, 15261 inserzioni) + push
2. `firebase deploy --only firestore:indexes --project calmify-388723` → 5 nuovi bio indexes building, 3 phantom indexes preserved (answered "N" to delete)
3. Phantom indexes reconciled (commit `abc6857`)
4. `gcloud builds submit --config=calmify-server/cloudbuild.yaml --project=calmify-388723` → build `ce63ee0a` SUCCESS 12m6s, revision live su `https://calmify-server-23546263069.europe-west1.run.app`
5. Smoke test: `/health` → "healthy" + 4 bio endpoint → 401 (auth attiva + routes registrate)
6. Flag flip `BackendConfig.BIO_REST = true` (commit `3244cde`) + push
7. Settings entries (BioContext + BioOnboarding) shipped (commit `0e0e4dc`)
8. HC permission runtime fix shipped (commits `1563dc6` + `8c51720`): `<intent-filter>` rationale + VIEW_PERMISSION_USAGE + `<uses-permission>` × 7 + `xml/health_permissions.xml` + `<meta-data>` su MainActivity + logging diagnostico
9. StepIntro 1:1 redesign con Claude Design (commit `5635b82`): eyebrow tag + 32sp display title + animated `BreathWaveVisual` Compose Canvas + fineprint + Skip footer
10. Device test su Samsung S24 + Android 15: 5-step pager funzionante → HC permission UI nativa → 7 data types granted → 118 step samples (samsung SM-S921B via android) ingestionati nella SQLDelight locale + visibili nel dashboard BioContextScreen "I tuoi dati, in locale"

**Lezione critica salvata** in `memory/feedback_health_connect_runtime_requirements.md`: tutti e 4 i pezzi del manifest HC (rationale intent-filter + VIEW_PERMISSION_USAGE intent-filter + uses-permission declarations + xml/health_permissions.xml con meta-data) sono RUNTIME requirements su Android 14+, non solo Play Store ship requirements. Mancare anche uno → silent fast-path abort del permission contract con 0 granted (HC non ha error surface osservabile per questo).

**Bio-Signal Phase 4 (server) CODE DONE 2026-05-11** (commit `9dc5a9b`, deploy now done 2026-05-12): server BioSignalService (4 ops with FIRESTORE_BATCH_LIMIT=500 chunked writes + deterministic doc IDs for idempotent upsert + IDOR-guard via principal.uid + final REVOKE audit on deleteAll), BioSignalRoutes (POST /ingest + GET /aggregate + DELETE /all + GET /export under `authenticate("firebase")`), 5 Firestore composite indexes pre-deployed in firestore.indexes.json (bio_aggregates 4 variants + bio_consent_log, all with `__name__ DESC` tie-breaker per lezione Phase 5), BioSignalNetworkClient interface in core/util + NoopBioSignalNetworkClient default + KtorBioSignalNetworkClient in data/network (resilient, swallows network errors), BackendConfig.BIO_REST flag (default false) wired in restOverrideModule with last-wins binding swap, BioSignalRepositoryImpl.syncPendingAggregates() real impl (dirty → POST → markClean first acceptedCount), deleteAll() local wipe → audit REVOKE → server fan-out best-effort. :calmify-server compile verde 31s, :app:assembleDebug verde 45s. **Polish/deferrals**: Cloud Run + indexes deploy + flag flip + smoke test E2E (NOT just /health, per CLAUDE.md regola 13).

**Bio-Signal Phase 3 (Onboarding) DONE 2026-05-11** (commit `2104923`): 5-step pager flow — Intro → DataTypes → Why → Permission → Confirm. Built in `:features:biocontext` alongside BioContext: `BioOnboardingContract` (MVI with Step enum + canGoNext computed property), `BioOnboardingViewModel` (state machine + auto-refresh of provider status on Permission step), `BioOnboardingScreen` (~430 LOC, AnimatedContent fade transitions, PagerTop with active/passed/inactive dots + skip link, 5 distinct step composables 1:1 with `design/biosignal/Calmify Bio Onboarding.html`), `BioOnboardingEntryPoint` (androidMain — `rememberLauncherForActivityResult(HealthConnectPermissions.createRequestPermissionResultContract())` for permission flow + Play Store deep-link with web fallback). 32 i18n keys EN+IT (chrome 4 + Intro 3 + DataTypes 9 + Why 7 + Permission 6 + Confirm 5). `Strings.BioOnboarding` facade with 32 typed getters. `HealthConnectPermissions` visibility changed `internal → public` (the only platform-bridge crossing data→feature boundary, by design — Android permission UIs need an Activity host). `features/biocontext/build.gradle` androidMain dep on `:data:mongo` + `health-connect-client` lib. Decompose: `RootDestination.BioOnboarding` + `Child.BioOnboarding` + `navigateToBioOnboarding()` + DecomposeApp rendering. `BioOnboardingViewModel` registered in `bioContextKoinModule`. `:app:assembleDebug` verde 43s. **Polish deferrals**: Skip prefs persistence + Settings entry point + device smoke test.

**Bio-Signal Phase 2.UI DONE 2026-05-11** (commit `57bdd97`): BioContextScreen transparency dashboard ("Privacy by Visibility") shipped. New `:features:biocontext` Gradle module + Decompose destination `RootDestination.BioContext` + `Child.BioContext` + `navigateToBioContext()`. 6-card layout (ProviderStatus pill + Inventory per-type + ConnectedSources tally + ServerNeverRaw promise + PrivacyActions export/delete + PrivacyStatement card) built 1:1 with Calmify design tokens (CalmifySpacing.xl/lg, CalmifyRadius.xxl/xl, MaterialTheme.colorScheme.surfaceContainerLow). BioContextViewModel reads from BioSignalRepository (30-day window, 7 data types) on init + refresh; effects ShareExport (FileProvider → ACTION_SEND), Toast, OpenHealthConnectSettings (intent fallback for SDK < 34). 36 i18n keys EN + IT (10 other locales fall through to EN per existing pattern); `Strings.BioContext` facade with 36 typed getters. `bioContextKoinModule` registered in `allKoinModules`. `:app:assembleDebug` verde 2m 54s. **Polish follow-ups**: Settings entry + Drawer entry + ES/FR/DE/PT translations + per-type toggle UI (currently revoke deep-links to Health Connect settings).

**Bio-Signal Phase 2 (data layer) DONE 2026-05-11** (commit `e3ceeb9`): SQLDelight schemas (`BioSignal.sq` raw samples with JSON-payload pattern + `BioAggregate.sq` rolling stats with dirty flag + `BioConsent.sq` GDPR Art.7 audit log). `BioSignalRepositoryImpl` (~390 LOC) — observeRawSamples Flow + getAggregate/observeAggregate + ingestFromProvider (reads 7 types, upserts raw, computes daily aggregates with mean/p10/p90/confidence-weighted) + syncPendingAggregates STUB returning 0 (Phase 4 will wire Ktor) + exportAll() Art.20 JSON + deleteAll() Art.17 atomic local + audit REVOKE event + TTL prune. `@Serializable` annotations on BioSignal sealed + 7 subtypes + DataConfidence + BioSignalSource + SleepStage + BioAggregate. Koin `bioSignalModule` exposes HealthDataProvider + BioSignalRepository singletons, registered in `allKoinModules`. `:app:assembleDebug` verde 17s. **UI deferred to 2.UI follow-up commit**: BioContextScreen + features/biocontext module + Strings.BioContext facade.

**Bio-Signal Phase 1 DONE 2026-05-11** (commit `b046f05`): Android Health Connect impl complete. 5 androidMain files in `data/mongo/.../biosignal/`: HealthConnectProvider (actual `HealthDataProvider`, read-only, `withContext(Dispatchers.IO)` everywhere, lazy client, SDK_AVAILABLE/UNAVAILABLE/PROVIDER_UPDATE_REQUIRED status mapping); HealthConnectMappers (Record→domain for HeartRate/HRV/Sleep+stages/Steps/RestingHR/SpO2/Exercise + inferConfidence + Metadata→BioSignalSource); HealthConnectPermissions (BioSignalDataType↔HC permission strings bidirectional + ActivityResultContract bridge); BioSignalSyncWorker (CoroutineWorker, 24h period + 4h flex, UNMETERED + battery-not-low constraints, idempotent + retry); BioCoverageDumper (debug-only Report struct with row-per-type, validation per Mi Band 10 + S24 worst-case in `memory/user_hardware.md`). Manifest `<queries>` for HC package visibility + ACTION_SHOW_PERMISSIONS_RATIONALE intent declaration. Deps `androidx.health.connect:connect-client:1.1.0-rc03` + `androidx.work:work-runtime-ktx:2.10.0` added to libs.versions.toml + data/mongo/build.gradle. `assembleDebug` verde 2m 56s. Rationale Activity + Privacy Policy update DEFERRED to Phase 3 onboarding. Device test (48h Mi Band wear) DEFERRED (not blocking Phase 2).

**Bio-Signal Phase 0 DONE 2026-05-11** (commit `064ab0e`): 4 foundation files in commonMain — BioSignal.kt (sealed hierarchy 7 types + DataConfidence + BioSignalSource + SleepStage + ActivityType + BioAggregate enums), HealthDataProvider.kt interface + ProviderStatus, BioSignalRepository.kt interface + IngestionResult, BioSignalProto.kt (7 raw Protos + BioAggregateProto + BioAggregateBatchProto + BioAggregateResponseProto + BioConsentEventProto). Build `:core:util` + `:shared:models` verde 27s. Phase 1 (Android Health Connect actual + WorkManager + tester Activity per Mi Band 10 / S24) next.

**R4 DONE 2026-05-11** (commit `2593eae`): theme README at `core/ui/.../theme/README.md` (171 LOC). Single source of truth doc covering 5 token files, usage examples, refactor rules, hard exclusions, Kotlin↔CSS mapping table. CompositionLocal deferred (unnecessary — plain object access sufficient for Motion/Spacing/Radius).

**R3.5-7 DONE 2026-05-11** (commit `c992a2a`): ProfileDashboard (1474 LOC), PaywallScreen (932 LOC), ComposerScreen (977 LOC), ChatBubble (284 LOC) refactorati 1:1 con CSS scale. Major user-visible surfaces ora tutti in scala canonical. **R3.8 tail DEFERRED**: i ~15-19 file minor restanti non-bloccanti per bio-signal Phase 0, possono procedere on-demand. R4 (theme README) come prossimo passo. Build verde: :features:profile (8s), :features:subscription (6s), :features:composer + :features:chat (18s).
