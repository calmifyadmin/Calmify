...
**Sprint i18n**: 🚧 **IN PROGRESS — Fase A→C.4 DONE 2026-04-19** (pre-Level 3, ~5-6 gg totali). Cumulative: home + search + social-profile + composer + messaging + thread-detail + avatar + insight + meditation = 102 keys × 12 lang + ~106 hardcoded migrati. Next: Fase C.5 (settings + habits + chat + SnapshotScreen). Vedi `memory/i18n_strategy.md`.
