...
**Sprint i18n**: ✅ **COMPLETE 2026-04-19 — Fases A→E** (pre-Level 3, 11 commit atomic). **140 keys × 12 lingue ≈ 1680 translation entries + ~145 hardcoded migrati su 31 file Kotlin** (27 feature + 4 core/social-ui). Modules: home, search, social-profile (+tail), follow-list, composer, messaging, thread-detail, avatar-creator, insight, meditation, snapshot, settings, chat, humanoid, core/social-ui quartet + Nav facade. Phase D: `values-en/` deleted, MainActivity.kt hardcoded preservato (app/ Android-only). Phase E: I18N_GUIDE.md consolidato + sprint summary table. Known deferrals: Noto fonts (before Level 3), full translation 6 new locales (on-demand), Detekt error mode (after 1 month). **Level 3 unblocked**. Vedi `memory/i18n_strategy.md`.
